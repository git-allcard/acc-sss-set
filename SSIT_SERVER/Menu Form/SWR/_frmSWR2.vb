...
Public Class _frmSWR2

    Public Shared isMemberRegistered As Boolean = True
    Dim xtd As New ExtractedDetails

    Private Sub _frmSWR2_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            GetEmailAddress()
    ...
    Private Sub _frmSWR2_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub btnNext_Click(sender As Object, e As EventArgs) Handles btnNext.Click
        ...
        '    SharedFunction.ShowWarningMessage("EMAIL ADDRESS IS REQUIRED.")
        'ElseIf Not email.IsMatch(txtEmail.Text.Trim) Then
        '    SharedFunction.ShowWarningMessage("INVALID EMAIL ADDRESS FORMAT.")
        'ElseIf txtConfirmEmail.Text = "" Then
        '    SharedFunction.ShowWarningMessage("CONFIRM EMAIL ADDRESS.")
        'ElseIf txtEmail.Text <> txtConfirmEmail.Text Then
        '    SharedFunction.ShowWarningMessage("EMAIL ADDRESS AND CONFIRM EMAIL ADDRESS ARE NOT IDENTICAL.")
        ...
        '    SharedFunction.ShowWarningMessage("USER ID LENGTH SHOULD BE 8-20 CHARACTERS.")
        'ElseIf IsNumeric(txtUserId.Text.Substring(0, 1)) Then
        '    SharedFunction.ShowWarningMessage("FIRST CHARACTER MUST BE ALPHABETIC.")
        'Else
        ''    Dim swr As New SWR
        ''    If swr.verifyUserAccount(txtUserId.Text) Then
        ''        If swr.verifyUserAccountResponse = "Y" Then
        ...
        ''        End If
        ''    Else
        ''        SharedFunction.ShowWarningMessage("FAILED TO VALIDATE USER ID." & vbNewLine & vbNewLine & swr.exceptions.ToUpper)
        ''    End If
        ''    swr = Nothing
        ''End If
    End Sub

    Private Sub lbEmail_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles lbEmail.LinkClicked
        txtEmail.Clear()
        txtEmail.Focus()
    End Sub

    Private Sub lbConfirmEmail_LinkClicked(sender As Object, e As LinkLabelLinkClickedEventArgs) Handles lbConfirmEmail.LinkClicked
    ...
    Public Sub SubmitSWR()
        'Dim swr As New SWR
        'If Not swr.registerUserAccount(txtUserId.Text, txtEmail.Text, SSStempFile) Then
        '    SharedFunction.ShowWarningMessage(swr.exceptions.ToUpper)
        'Else
        '    If swr.registerUserAccountResponse.ToUpper = "SUCCESS" Then
        '        _frmUserAuthentication.getTransacNum()
        '        Dim inMat As New insertProcedure
        '        inMat.insertSimplifiedWebRegistration(SSStempFile, txtEmail.Text, txtUserId.Text, _frmUserAuthentication.lblTransactionNo.Text, HTMLDataExtractor.MemberFullName)
        '        _frmSWR3.lblMessage.Text = _frmSWR3.lblMessage.Text.Replace("@email", txtEmail.Text).Replace("@trn", _frmUserAuthentication.lblTransactionNo.Text)
        '        _frmMainMenu.splitContainerControl.Panel2.Controls.Clear()
        ...
        '        Me.Hide()
        '    Else
        '        SharedFunction.ShowWarningMessage(swr.registerUserAccountResponse.ToUpper)
        '        _frmMainMenu.splitContainerControl.Panel2.Controls.Clear()
        '        Me.TopLevel = False
    ...
    Private Sub GetEmailAddress()
        Dim slMobileWS2BeanService As New SalaryLoan.slMobileWS2BeanService
        Try
            If slMobileWS2BeanService.getContactInfoAddress(SSStempFile) Then
                txtEmail.Text = slMobileWS2BeanService.getContactInfoAddressResponse.email.Trim
                txtConfirmEmail.Text = slMobileWS2BeanService.getContactInfoAddressResponse.email.Trim
            End If
        Catch ex As Exception
            txtEmail.Text = ""
    ...
    Private Sub txtEmail_Click(sender As Object, e As EventArgs) Handles txtEmail.Click, txtConfirmEmail.Click, txtUserId.Click
